...
class Postgres extends AbstractImplementation
{
    /**
     * @var static $instance
     */
     ...
     */
    public static function initPostgres(array $initialData, $serverName, $database, $username, $password, array $dependencies)
    {
        if (!static::$instance) {
            ...
            static::$instance->connection = new PDO('pgsql:host=' . $serverName. ';dbname=' . $database, $username, $password);
            ...
            foreach ($initialData as $table => $data) {
                $query = "SELECT a.attname "
                    ...
                    . "WHERE i.indrelid = 'hrp_company'::regclass "
                    . "AND   i.indisprimary;";
                $stmt = static::$instance->connection->prepare($query);
                $stmt->execute();
                static::$instance->primaryKeys[$table] = [];
                foreach ($stmt->fetchAll() as $row) {
                    static::$instance->primaryKeys[$table][] = $row['attname'];
                }
            }
            ...
            throw new AlreadyInitializedException('Postgres library already initialized');
        }
    }
